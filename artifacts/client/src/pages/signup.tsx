...
import { PlaneTakeoff, User, Mail, Lock, Phone, MapPin, Globe, Camera, ChevronLeft, Sparkles } from "lucide-react";
...
export default function Signup() {
  ...
  const watchFields = form.watch(["firstName", "lastName", "city", "country"]);
  const fullName = `${watchFields[0]} ${watchFields[1]}`.trim();
  const location = [watchFields[2], watchFields[3]].filter(Boolean).join(", ");
  ...
  const onSubmit = (values: SignupFormValues) => {
    ...
    signupMutation.mutate(
      { data: { name, email: values.email, password: values.password, avatarUrl: avatarPreview || undefined } },
      {
        onSuccess: (data) => {
      ...
      >
        ...
        <div className="relative z-10 flex flex-col h-full p-8 xl:p-10">
          ...
          <div className="flex-1 flex flex-col justify-center">
            {avatarPreview || fullName ? (
              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col items-center text-center">
                  <div className="relative mb-6">
                    <div className="w-24 h-24 rounded-full overflow-hidden ring-4 ring-sky-400/30 border-2 border-white/20 shadow-xl bg-sky-900/50 flex items-center justify-center">
                      {avatarPreview ? (
                    ...
                    <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-sky-500 border-2 border-sky-900 flex items-center justify-center shadow-lg">
                  ...
                  <p className="text-sky-300 font-medium text-sm mt-1 flex items-center gap-1.5 justify-center">
                  ...
                  <div className="grid grid-cols-2 gap-4 w-full">
                    <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                      <p className="text-[10px] text-sky-200/50 uppercase font-bold tracking-wider">Status</p>
                      <p className="text-white text-xs font-semibold mt-0.5">Ready to fly</p>
                    </div>
                    <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                      <p className="text-[10px] text-sky-200/50 uppercase font-bold tracking-wider">Plan</p>
                      <p className="text-white text-xs font-semibold mt-0.5">Free Member</p>
                    </div>
                  </div>
                </div>
              </div>
